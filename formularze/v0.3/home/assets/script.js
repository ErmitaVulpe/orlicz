import { dbCheck, logout } from "../../assets/utils.js";
dbCheck();
...
Object.keys(calcsTemplateDB).forEach(title => {
    ...
    sideMenu.innerHTML += `
        <div class="side-menu-group side-menu-group-not-fav">
            <div class="side-menu-group-title">
                <div class="side-menu-group-title-hr"></div>
                <div class="side-menu-group-title-content">${title}</div>
            </div>
        </div>`
    const sideMenuGroup = document.getElementsByClassName("side-menu-group")[document.getElementsByClassName("side-menu-group").length - 1];

    let temp = `<div class="home-grid-container">`;
    let sideTemp = `<div class="side-menu-group-content">`
    ...
    calcsTemplateDB[title].forEach(element => {
        ...
        sideTemp += `<div class="side-menu-group-content-element" id="side-${element}">${element}</div>`
    });
    temp += `</div>`
    sideTemp += `</div>`

    notFavouritesElement.innerHTML += temp;
    sideMenuGroup.innerHTML += sideTemp;
});
...
function copyANode(title) {
    if (document.getElementById("favourites").innerHTML === "") {
        ...
        document.getElementById("side-favourites").classList.remove("hidden");
    };
    ...
    let sideMenuFavouritesContent = document.getElementById("side-favourites").getElementsByClassName("side-menu-group-content")[0];
    let sideMenuFavouritesContentHold = sideMenuFavouritesContent.innerHTML
    sideMenuFavouritesContent.innerHTML = `<div class="side-menu-group-content-element" id="side-favourites-${title}">${title}</div>` + sideMenuFavouritesContentHold;
    
    if (favourites[favourites.length - 1] == title) {
        Array.from(sideMenuFavouritesContent.getElementsByClassName("side-menu-group-content-element")).forEach(element => {
            element.addEventListener("click", function () {
                calcPopup(element.innerText);
            });
        });
    }
...
function makeTheFuckingStarWork(star) {
    star.addEventListener("click", function (event) {
        ...
        if (calcsBackedDB[elementTitle].length === 1) {
            ...
            const newNode = copyANode(thisTitle)
            makeShitWork(newNode);
            ...
            calcsBackedDB[elementTitle][1].remove();
            calcsBackedDB[elementTitle].splice(1, 1);
            // .parentElement.removeChild(calcsBackedDB[elementTitle][1])
            ...
            document.getElementById(`side-favourites-${elementTitle}`).remove()

            localStorage.setItem("db-users", JSON.stringify(db));

            if (favourites.length === 0) {
                document.getElementById("favourites").innerHTML = "";
                document.getElementById("side-favourites").classList.add("hidden");
            }
        }
...
function calcPopup(name) {
    if (openedCalc === name) {
        document.getElementById(name).classList.remove("active");
        openedCalc = "";
    } else if (openedCalc) {
        document.getElementById(openedCalc).classList.remove("active");
...
Array.from(notFavouritesElement.getElementsByClassName('element')).forEach(element => {
    element.addEventListener("click", function () {
        calcPopup(element.getElementsByClassName("element-title")[0].innerText);
    });
});

Array.from(document.getElementsByClassName("side-menu-group-not-fav")).forEach(element => {
    Array.from(element.getElementsByClassName('side-menu-group-content-element')).forEach(element => {
        element.addEventListener("click", function () {
            calcPopup(element.innerText);
        });
    });
...
// let lolo = ""
// Object.keys(calcsTemplateDB).forEach(element => {
//     calcsTemplateDB[element].forEach(element2 => {
//         lolo += `<div class="calc-container" id="${element2}">
//     <span class="calc-title">${element2}</span>
// </div>`
//     });
// });
